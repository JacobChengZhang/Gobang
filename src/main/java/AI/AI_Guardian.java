...
public class AI_Guardian implements AiMove {
  ...
  private PieceQuery pieces;
  private final int[][] p; // analog pieces
  private final int order = Utils.getOrder();

  private final int depth = 3; // include 0, i.e. [0, depth]
  private _Piece bestMove = null;
  ...
  public AI_Guardian(int color, PieceQuery pieces) {
    this.color = color;
    this.pieces = pieces;
  ...
  public Piece nextMove() {
    ...
    if (!checkIfPieceExist()) {
      return Piece.createPieceByAI((order - 1) / 2, (order - 1) / 2, color);
    }

    alphaBeta(this.depth, Integer.MIN_VALUE, Integer.MAX_VALUE, true, 0, 0, 0);

    if (bestMove != null) {
      return Piece.createPieceByAI(bestMove.x, bestMove.y, color);
    } else {
      return null;
    }
  }
  ...
  @Override
  public void gameEnds(GameState ending) {
  ...
  private int alphaBeta(int depth, int alpha, int beta, boolean isMaximizingPlayer, int x, int y, int color) {
    ...
    if (isMaximizingPlayer) {
      ...
      ArrayList<_Piece> moveList = generateLegalMoves(this.color);
      for (_Piece _pi : moveList) {
  ...
  private ArrayList<_Piece> generateLegalMoves(int color) {
    ArrayList<_Piece> moveList = new ArrayList<>();
    for (int x = szLowestX; x <= szHighestX; x++) {
      for (int y = szLowestY; y <= szHighestY; y++) {
        if (p[x][y] == 0) {
          //moveList.add(new _Piece(x, y, color, evaluateOneMove(x, y, color)));
          moveList.add(new _Piece(x, y, color));
        }
      }
    }
    return moveList;
